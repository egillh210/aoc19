...
const tinput = `R8,U5,L5,D3
U7,R6,D4,L4`

const tinput1 = `R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83`

const tinput2 = `R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7`

const move = (startpos, instruction) => {

    let [x,y] = startpos.split(',').map(Number)

    const [,direction,num] = instruction.match(/(\w)(\d+)/)

    switch(direction) {

        case 'R': {
            const n = Array(Number(num)).fill(x+1).map((v,i) => `${v+i},${y}`)
            return n;
        }
        case 'L': {
            const n = Array(Number(num)).fill(x-1).map((v,i) => `${v-i},${y}`)
        ...
        case 'U': {
            const n = Array(Number(num)).fill(y+1).map((v,i) => `${x},${v+i}`);
            return n
        }
        case 'D': {
            const n = Array(Number(num)).fill(y-1).map((v,i) => `${x},${v-i}`);
...
const part2 = inputstr => {
    ...
    const path1 = arr[0].split(',');
    const path2 = arr[1].split(',');

    let pos1 = o;
    let pos2 = o;

    for(const instr of path1) {
        const res = move(pos1, instr)
        pos1 = res[res.length-1];
        arr1 = [...arr1, ...res]
    }

    for(const instr of path2) {
        const res = move(pos2, instr)
        pos2 = res[res.length-1];
        arr2 = [...arr2, ...res]
    }
    ...
    for(const item of intersection) {
        obj[item] = arr1.indexOf(item) + arr2.indexOf(item) + 2;
    }
...
const part1 = inputstr => {
    ...
    for(const instr of path1) {
        const res = move(pos1, instr)
        pos1 = res[res.length-1];
        set1 = new Set([...set1, ...res])
    }

    for(const instr of path2) {
        const res = move(pos2, instr)
        pos2 = res[res.length-1];
        set2 = new Set([...set2, ...res])
    }
    ...
    return [...intersection].map(point => manhattanDistance(point, '1,1')).reduce((acc, val) => {
